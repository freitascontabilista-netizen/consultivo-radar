      ...
      <header className="sticky top-0 z-10 border-b border-border/60 bg-background/80 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden items-center font-bold tracking-tight sm:flex" style={{ height: "36px", color: "#0A2647", fontSize: "22px", lineHeight: "36px" }}>
              Freitas Consultoria
            </div>
            <UserMenu />
          </div>
        ...
        {loading && !cliente ? (
          ...
          <>
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <div className="flex items-center gap-3">
                  <h1 className="truncate text-2xl font-semibold tracking-tight">{cliente.razao_social ?? "Cliente"}</h1>
                  <button onClick={openEdit} className="inline-flex items-center gap-1 rounded-md bg-blue-900 px-2 py-1 text-xs text-white hover:bg-blue-800 transition-colors">
                    <Pencil className="h-3 w-3" /> Editar cadastro
                  </button>
                </div>
                {cliente.nome_fantasia && <p className="mt-1 text-sm text-muted-foreground">{cliente.nome_fantasia}</p>}
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <StatusBadge status={cliente.semaforo} />
                  {cliente.segmento && <span className="inline-flex items-center rounded-full bg-muted px-2.5 py-1 text-xs font-medium text-foreground/70">{cliente.segmento}</span>}
                  {cliente.regime_tributario && <span className="inline-flex items-center rounded-full bg-muted px-2.5 py-1 text-xs font-medium text-foreground/70">{cliente.regime_tributario}</span>}
                </div>
              ...
              <div className="rounded-lg border border-border/60 bg-card px-5 py-3 text-right">
                <div className="text-xs uppercase tracking-wide text-muted-foreground">Dias sem orientação</div>
                <div className={cn("text-3xl font-semibold tabular-nums", dias >= 60 ? "text-status-critical" : dias >= 30 ? "text-status-warning" : "text-status-success")}>{dias}</div>
              </div>
            </div>
                      ...
                      {interacoesPaginadas.map((it) => {
                        ...
                        return (
                          <li key={String(it.id)} className="p-5">
                            ...
                            <p className="mt-2 text-sm font-medium text-foreground">{it.assunto ?? "Sem assunto"}</p>
                            {it.resumo && <p className="mt-1 text-sm text-muted-foreground">{it.resumo}</p>}
                          </li>
                        );
                      })}
                    </ul>
            ...
            <section className="mt-10">
              ...
              <Card className="overflow-hidden border-border/60 shadow-none">
                {acoes.length === 0 ? (
                  <div className="p-8 text-center text-sm text-muted-foreground">Nenhuma ação aberta.</div>
                ) : (
                  <ul className="divide-y divide-border/60">
                    {acoes.map((ac) => {
                      ...
                      return (
                        <li key={String(ac.id)} className="p-5">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <span className={cn("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium", ucfg.classes)}>{ucfg.label}</span>
                          </div>
                          {ac.problema_identificado && <p className="mt-2 text-xs uppercase tracking-wide text-muted-foreground">Problema identificado</p>}
                          {ac.problema_identificado && <p className="text-sm text-foreground/80">{ac.problema_identificado}</p>}
                          {ac.acao_recomendada && <p className="mt-2 text-xs uppercase tracking-wide text-muted-foreground">Ação recomendada</p>}
                          {ac.acao_recomendada && <p className="text-sm text-foreground/80">{ac.acao_recomendada}</p>}
                          {ac.data_retorno_prevista && <p className="mt-3 text-xs text-muted-foreground">Retorno previsto: {formatDate(ac.data_retorno_prevista)}</p>}
                          <button
                            onClick={async () => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl">
            ...
            <div className="flex justify-end gap-3 border-t px-6 py-4">
              <button onClick={() => setEditOpen(false)} className="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Cancelar</button>
              <button onClick={saveEdit} disabled={saving} className="rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700 disabled:opacity-50">
